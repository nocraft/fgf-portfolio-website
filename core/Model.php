...
class Model{
	...
	**/
	public function find($req = array()){
       ...
       if(isset($req['join'])){
          $sql .= strtoupper((string)$req['join']).' '.'JOIN ';
          if(isset($req['on'])){
            $cond = array();
        ...
        //Construction de la condition avec symbole
        if(isset($req['conditions'])){
            ...
            }else
            {
				$cond = array();
				$cond2 = array();
				if (isset($req['symbole']))
				{
				$symbole = explode(',', $req['symbole']);
				}
				$u=0;
                foreach($req['conditions'] as $k=>$v){
				  ...
				  $u++;
               	...
               	if(isset($req['conditions']['search']) && $k == 'search'){
               		foreach ($req['conditions']['search'] as $w) {
               			foreach ($w as $x => $y) {
               				$y = addslashes($y);
               		   		$cond2[] = "$x "."LIKE"." '$y'";
               				//debug($y);
                ...
                $sql .= implode(' AND ',$cond);
                if (isset($cond2) && !empty($cond2)) {
               		$sql .= " AND ".implode(' OR ',$cond2);
               	}
	...
	**/
	public function findCount($conditions,$symbole = null) {
	//debug($conditions);
	//debug($symbole);
	...
	**/
	function findList($condition = null, $req = array()){
		if(!isset($req['fields'])){
